...
export default function SarpaPage({ onBack, onOpenProduct, onOpenCart }) {
  const { count, total } = useCart();
  const [tab, setTab] = useState('obligatory');
  const [data, setData] = useState({ obligatory: [], recommended: [] });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetchSarpa()
      .then((d) => setData({ obligatory: d.obligatory || [], recommended: d.recommended || [] }))
      .catch(console.error)
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <h1 style={s.title}>Соберите сарпу</h1>
        <div style={{ width: 40 }} />
      </div>
...
const s = {
  ...
  skeleton: { height: 102, borderRadius: 16, background: '#F0E6CC', marginBottom: 10 },
